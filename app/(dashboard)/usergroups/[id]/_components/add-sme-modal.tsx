...
import { useSearchUserGroupBusinesses, useAssignBusinessesToGroup, type BusinessSearchItem } from "@/lib/api/hooks/user-groups-businesses";
...
export default function AddSmeModal({ open, onOpenChange, groupId, onAdded }: AddSmeModalProps) {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      if (result.assigned > 0) {
        toast.success(`Successfully assigned ${result.assigned} business${result.assigned > 1 ? 'es' : ''}`);
      }
      if (result.skipped > 0) {
        toast.warning(`${result.skipped} business${result.skipped > 1 ? 'es were' : ' was'} already in the group`);
      }
      if (result.invalid && result.invalid.length > 0) {
        toast.error(`${result.invalid.length} invalid business ID${result.invalid.length > 1 ? 's' : ''}`);
      }
      ...
      toast.error(error?.response?.data?.message || "Failed to assign businesses");
    }
  };
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-[900px] p-0 overflow-hidden max-h-[90vh] flex flex-col">
        <div className="px-8 py-6 flex-shrink-0">
          <DialogHeader>
            <DialogTitle className="text-2xl font-medium text-midnight-blue">Add Businesses</DialogTitle>
            <DialogDescription className="text-primaryGrey-500">
              Search and select businesses to add to this group. Businesses already in the group are marked and cannot be selected.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="mt-6 border rounded-md">
            ...
            <div className="max-h-96 overflow-auto">
              {isLoading ? (
                ...
                <div className="px-4 py-8 text-center text-sm text-primaryGrey-500">
                  {search ? "No businesses found" : "Start typing to search for businesses"}
                </div>
              ) : (
                <>
                  {businesses.map((business) => {
                    const isSelected = selected.some((b) => b.id === business.id);
                    ...
                    return (
                      <button
                        ...
                        } ${isAlreadyInGroup ? "opacity-60 cursor-not-allowed" : "cursor-pointer"}`}
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="flex-shrink-0">
                            {isAlreadyInGroup ? (
                              <Badge variant="outline" className="text-xs border-green-500 text-green-500 bg-green-50">
                                Already in Group
                              </Badge>
                            ) : isSelected ? (
                              <Badge variant="outline" className="text-xs border-green-500 text-green-500 bg-green-50">
                  ...
                  {totalPages > 1 && (
                    <div className="px-4 py-3 border-t flex items-center justify-between">
                      <p className="text-xs text-primaryGrey-500">
                        Page {page} of {totalPages} • {searchResults?.pagination?.total || 0} total
                      </p>
                ...
                <div className="flex flex-wrap gap-2">
                  {selected.map((business) => (
                    ...
                    >
                      {business.name}
                      <button
                        onClick={() => setSelected((prev) => prev.filter((b) => b.id !== business.id))}
        ...
        <DialogFooter className="mt-auto border-t pt-6 px-8 pb-6 flex items-center justify-end gap-3 flex-shrink-0">
          ...
          >
            {assignMutation.isPending ? "Adding..." : `Add ${selected.length > 0 ? `${selected.length} ` : ""}Business${selected.length !== 1 ? 'es' : ''}`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
