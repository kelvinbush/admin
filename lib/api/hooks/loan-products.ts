    ...
    "/loan-products",
    {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: queryKeys.loanProducts.lists(),
        });
      },
    },
  );
...
export function useUpdateLoanProduct() {
  ...
  return useClientApiMutation<
    LoanProduct,
    { id: string; data: UpdateLoanProductData }
  >(
    (
      api: any,
      variables: {
    ...
    ) => api.patch(`/loan-products/${variables.id}`, variables.data),
    {
      onSuccess: (
        data: LoanProduct,
        variables: { id: string; data: UpdateLoanProductData },
      ) => {
        queryClient.invalidateQueries({
          queryKey: queryKeys.loanProducts.lists(),
        });
        queryClient.invalidateQueries({
          queryKey: queryKeys.loanProducts.detail(variables.id),
        });
      },
    },
  );
...
export function useDeleteLoanProduct() {
  ...
  return useClientApiDelete<{ success: boolean }>("/loan-products", {
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: queryKeys.loanProducts.lists(),
      });
