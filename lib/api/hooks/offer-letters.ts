    ...
    "/offer-letters",
    {
      onSuccess: (data: OfferLetterItem) => {
        queryClient.invalidateQueries({
          queryKey: queryKeys.offerLetters.lists(),
        });
        // Also invalidate loan application data since it includes offer letters
        queryClient.invalidateQueries({
          queryKey: queryKeys.loanApplications.detail(data.loanApplicationId),
        });
        queryClient.invalidateQueries({
...
export function useSendOfferLetter() {
  ...
  >(
    ...
    {
      onSuccess: (
        data: SendOfferLetterResponse,
        variables: { id: string; data: SendOfferLetterData },
      ) => {
        queryClient.invalidateQueries({
          queryKey: queryKeys.offerLetters.lists(),
        });
        queryClient.invalidateQueries({
          queryKey: queryKeys.offerLetters.detail(variables.id),
        });
  ...
  >(
    ...
    ) => api.patch(`/offer-letters/${variables.id}`, variables.data),
    {
      onSuccess: (
        ...
        variables: { id: string; data: UpdateOfferLetterData },
      ) => {
        queryClient.invalidateQueries({
          queryKey: queryKeys.offerLetters.lists(),
        });
